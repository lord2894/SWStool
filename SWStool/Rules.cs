...
namespace TermRules
{
    public class Rules
    {
        public Rules(string inputfile) {
            find = new FindFunctions();
            aux = new AuxiliaryFunctions();
            proc = new TermsProcessing(inputfile);
        }
        FindFunctions find;
		        ...
		        if (k == -1)
		        {
			        if (AuthTermsAr.TermsAr[i].Pos.Count>0)
			        {
				        ...
				        for (int j = 1; j < AuthTermsAr.TermsAr[i].Pos.Count; j++)
				        {
					        ...
					        e.indexElement = i;
					        MainArrayTermsAr.TermsAr[i].Pos.Add(e);
				        }
			        }
		        }
	        }
		        ...
		        if (res != -1)
		        {
			        ...
			        for (int j = 0; j < NonDictTermsAr.TermsAr[i].Components.Count; j++)
			        {
				        ...
				        for (int k = 0; k < NonDictTermsAr.TermsAr[i].Components[j].Count; k++)
				        {
					        ...
					        int r = find.findINList(DictTermsAr.TermsAr, NonDictTermsAr.TermsAr[i].Components[j][k]);
					        if (r != -1)
					        {
        ...
        public void Rule4_Msyn_to_M(Terms MainTermsAr, SynTerms SynTermsAr)
        {
	        ...
	        {
		        int first_alt = find.findINList(MainTermsAr.TermsAr, SynTermsAr.TermsAr[i].alternatives.first);
		        int second_alt = find.findINList(MainTermsAr.TermsAr, SynTermsAr.TermsAr[i].alternatives.second);
		        if (first_alt != -1 && second_alt == -1)
		        {
			        SynTermsAr.TermsAr[i].setToDel = true;
			        Term newEl = new Term();
			        newEl.frequency = MainTermsAr.TermsAr[first_alt].frequency;
			        newEl.TermWord = SynTermsAr.TermsAr[i].alternatives.second;
			        newEl.setToDel = false;
		        ...
		        else if (first_alt == -1 && second_alt != -1)
		        {
			        ...
			        newEl.TermWord = SynTermsAr.TermsAr[i].alternatives.first;
			        newEl.setToDel = false;
                    newEl.TermFragment = SynTermsAr.TermsAr[i].TermFragment;
			        MainTermsAr.TermsAr.Add(newEl);
